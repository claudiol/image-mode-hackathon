...
variable "image_builder_cockpit_port" {
  type        = number
  default     = 9090
  description = "TCP port exposed for the Image Builder Cockpit web interface."
...
variable "image_builder_cockpit_allowed_cidrs" {
  type        = list(string)
  description = "IPv4 CIDR blocks allowed to access the Image Builder Cockpit web interface."
  ...
  validation {
    condition = alltrue([
      for cidr in var.image_builder_cockpit_allowed_cidrs :
      can(cidrhost(cidr, 0))
    ])

    error_message = "Every entry in image_builder_cockpit_allowed_cidrs must be a valid IPv4 CIDR block."
  }
}
...
variable "satellite_compute_profile_name" {
  type        = string
  default     = "AWS POC"
  description = "Name of the general Satellite compute profile used for AWS EC2 provisioning."
...
variable "satellite_image_builder_compute_profile_name" {
  ...
  description = "Name of the Satellite compute-profile configuration used for Image Builder EC2 instances."

  validation {
    condition = (
      trimspace(var.satellite_image_builder_compute_profile_name) != ""
    )

    error_message = "satellite_image_builder_compute_profile_name cannot be empty."
  }
}
...
variable "satellite_default_compute_profile_name" {
  ...
  description = "Name of the Satellite compute-profile configuration used for generic Image Mode EC2 instances."

  validation {
    condition = (
      trimspace(var.satellite_default_compute_profile_name) != ""
    )

    error_message = "satellite_default_compute_profile_name cannot be empty."
  }
}